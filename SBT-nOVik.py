...
def find_similar_definitions_tfidf(definitions, stopwords, threshold=0.4):
    ...
    definition_texts = [preprocess_text(definition[0], ignore_stopwords) for definition in definitions]
    original_texts = [definition[0] for definition in definitions]  # Сохраняем оригинальные тексты
    ...
    for i in range(len(definition_texts)):
        for j in range(i + 1, len(definition_texts)):  # j начинается с i + 1, чтобы избежать повторного сравнения и сравнения с самим собой
            ...
            if similarity > threshold and similarity < 0.9:  # Исключаем пары с идентичными определениями (схожесть < 0.9)
                pair = tuple(sorted([original_texts[i], original_texts[j]]))  # Используем оригинальные тексты
    ...
    filtered_pairs = [
        ...
        if not any(stopword in def1.lower() for stopword in stopwords) and not any(stopword in def2.lower() for stopword in stopwords)
    ]
        ...
        with conn.cursor() as cursor:
            ...
            with open(filtered_csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for definition1, definition2, similarity in similar_pairs:
                    csv_writer.writerow([definition1])
                    csv_writer.writerow([definition2])
